...
void ui_draw_button(int x, int y, int size, const char *text, bool pressed)
{
    ...
    // Draw button border
    display_draw_rectangle(x, y, size, 2, border_color); // Top
    display_draw_rectangle(x, y, 2, size, border_color); // Left
    display_draw_rectangle(x + size - 2, y, 2, size, border_color); // Right
    display_draw_rectangle(x, y + size - 2, size, 2, border_color); // Bottom
...
{
    display_draw_string(x, y, text, color);
}
...
{
    // no periodic UI work needed yet
}
